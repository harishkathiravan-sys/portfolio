...
export function SceneLighting({ progress }: SceneLightingProps) {
  ...
  useFrame(() => {
    ...
    if (keyRef.current) keyRef.current.intensity = Math.min(p * 1.2, 0.6);
    if (rimRef.current) rimRef.current.intensity = Math.min(p * 0.8, 0.4);
    if (ambientRef.current) ambientRef.current.intensity = 0.05 + p * 0.15;
  });
  ...
  return (
    <>
      ...
      <directionalLight position={[-3, 2, 4]} intensity={0.15} color="#e0e0f0" />
